...
public class SearchClient {
    ...
    public State[] latestStateArray = null;
    public String[] latestServerOutput = null;
    private Boolean[] agentErrorState = null; // List of Booleans used to keep track of errors when sending plans to the server
    ...
    public SearchClient(BufferedReader serverMessages) throws Exception {
        ...
        agentErrorState = new Boolean[agentList.size()];
        ...
        while (!finished) {
            ...
            for (Agent agent : agentListByPriority) {
                if ((agent.getCurrentGoal() != null) || agent.moveToCornerCaseGoal || agent.destinationGoal != null && !agent.getAgentOrder()) { //An agent with an order to wait during clearing don't search for its goal for 1 turn
                    ...
                    if (plan != null && !agent.isWaiting & !agent.getAgentOrder()) {
                        ...
                        /*
                        // Show previous Plan
                        System.err.println("Previous Stored Plan for agent " + agent.getId() +":\n");
                        for (int i = 0; i < previousPlan.size(); i++){
                            System.err.println(previousPlan.get(i).toString());
                        }
                        */
                        minLength = plan.size();
            ...
            while (cont) {
                ...
                // If error, we get out of the loop  and we stop executing the plans
                /*
                if (!status) {
                    System.err.println("Unsolved level - Exit 1");
                    System.exit(0);
                }
                */
            ...
            for (int i = 0; i < latestServerOutput.length; i++) {
                ...
                if (latestServerOutput[i] != null && latestServerOutput[i].equals("false")) {
                    ...
                    // Find agent concerned by conflict
                ...
                // Case when Agent action is not part of the error    
                } else {
                    ...
                    for (Agent a : agentList) {
                        if ((a.getAgentOrder()) && (a.getAgentWhoOrdered().getId() == succeedAgent.getId())) {
                            a.setOrder(false);
                        }
                    }
                }
             ...
             /*
            //Print in console for tests 
            for (Agent a : agentList) {
                System.out.println(a.getAgentOrder());
            }
            */
            
            // If there was a conflcit
            // Send plans of agents until all conflicts are resolved
            if (conflict_error) {
                while (sendNextStepToServer(serverMessages)) {
                    boolean status = State.updateStaticMap(latestStateArray, latestServerOutput);

                    // If we still have an error, we get out of the loop and we stop executing the plans
                    // We can't resolve this level
                    /*
                    if(!status) {
                        System.err.println("Unsolved level - Exit 2");
                        System.exit(0);
                    } 
                    */
    ...
    */
    ...
    /** -------------- START: Functions for communication between Agents -------------- */

    public Command remove_action_from_plan(Agent agent) {
        /* // Old version taking using the plan attribute from agent
        if(agent.plan.size() > 0){
            State next = agent.plan.get(0);
            agent.lastStateHelp = next;
			agent.plan.remove(0); // Remove first element once we read agent action
            return next.action;
        */  
        if(planByAgent.get(agent).size()  > 0){
            State next = planByAgent.get(agent).get(0);
            agent.lastStateHelp = next;
			planByAgent.get(agent).remove(0); // Remove elemnts once we read action
            return next.action;
		} else {
			return null;
		}
    ...
    public void askHelp(Agent agentAskingHelp, State currentState, State[] lastStatesByAgentArray) {
        ...
        // Show helped agent plan before removing steps
        /*// Old version taking using the plan attribute from agent
        System.err.println("Show the current plan of Agent asking help:\n");
        for (int i = 0; i < agentAskingHelp.plan.size(); i++){
            System.err.println(agentAskingHelp.plan.get(i).toString());
        ...
        */
		...
		if(agentAskingHelp.lastStateHelp != null) {
            action_agent = agentAskingHelp.lastStateHelp.action; // Get last attempted action
        ...
        // Store all positions of Agent and Boxes for each action in the agent plan
		action_agent = remove_action_from_plan(agentAskingHelp); // Get the last cmd from the plan of the agent
		while(action_agent != null){
			//System.err.println(action_agent.toString());
			actionEffectsCoordinates = currentState.action.commandToCoordinates(agent_coordinates, action_agent);
			agent_coordinates = new Coordinate(actionEffectsCoordinates.get(0).getRow(), actionEffectsCoordinates.get(0).getColumn());
            clearCoordinates.addAll(actionEffectsCoordinates);
            System.err.println("Last action in askHelp:" + action_agent.toString());
            System.err.println("Last State in askHelp:\n" + agentAskingHelp.lastStateHelp.toString()); // agentAskingHelp.lastStateHelp.toString()
			action_agent = remove_action_from_plan(agentAskingHelp);
        }
        
        // Rearrange plan
        planByAgent.replace(agentAskingHelp, plan_helped);


        // Check the plan of all other agents
        // Check for each of them if a movable box of the same color or the agent itself is on a cell which need to be cleared
		for(Agent agent : agentList){
			if(agent.getId() != agentAskingHelp.getId()){
				ArrayList<Coordinate> agentClearCoordinates = new ArrayList<>();
				for(Coordinate coord : clearCoordinates){
                    System.err.println("Last State of Agent " + agent.getId() + ":\n" + lastStatesByAgentArray[agentList.indexOf(agent)].toString());

                    BoardObject boardobject = State.realBoardObjectsById.get(lastStatesByAgentArray[agentList.indexOf(agent)].getlocalIdByCoordinate().get(coord));
                    Box annoying_box = null;
                    if (boardobject instanceof Box){
                        annoying_box = (Box) boardobject;
                    }
                    ...
                    if(lastStatesByAgentArray[agentList.indexOf(agent)].getParent().getLocalCoordinateById().get(agent.getId()).equals(coord) || // add getParent() ?
                    ...
                    /* TODO: Maybe two agents can come to clear the same cells with a box on it. Pick only one of them to do it if it is the case */
                }

                // Show helping agent plan
                /* // Old verison with plan as attribute
                System.err.println("Show the current plan of helping Agent:\n");
                for (int i = 0; i < agent.plan.size(); i++){
                    System.err.println(agent.plan.get(i).toString());
                }
                */ 
                // Show helping agent plan
                System.err.println("Show the current plan of Agent who will help:\n");
                for (int i = 0; i < planByAgent.get(agent).size(); i++){
                    System.err.println(planByAgent.get(agent).get(i).toString());

                }

                // Add last removed State to plan
                //planByAgent.get(agent).add(0, planByAgent.get(agent).get(0).getParent());

                // Show helping agent plan
                System.err.println("Show the current plan of Agent who will help after adding first State:\n");
                for (int i = 0; i < planByAgent.get(agent).size(); i++){
                    System.err.println(planByAgent.get(agent).get(i).toString());

                }
                
                // Give to each agent a list of cells to Clear
				clearCells(agentClearCoordinates, currentState, agent, agentAskingHelp, lastStatesByAgentArray);
			}
		}
    }


    public void clearCells(ArrayList<Coordinate> coordinatesToClear, State currentState, Agent agentHelping, Agent agentAskingHelp, State[] lastStatesByAgentArray){
        
        // To show the clearing process
        System.err.println("Agent number " + agentHelping.getId() + " attempting to clear path for agent" + agentAskingHelp.getId());
        
        for(Coordinate coord : coordinatesToClear){
            System.err.println("Coordinates to clear: " + coord.toString());
        }
        
                
        // Clear Agent helper Plan and add coordinates to clear
        agentHelping.isClearing  = true;
        /* // Old version with plan as attribute
        agentHelping.plan.clear(); */
        planByAgent.get(agentHelping).clear();
        agentHelping.clearCoordinates = coordinatesToClear;

        // Copy of the State of the agent asking for help
        // Old Versions:
        //State myCurrentState = currentState.getParent().getCopy(); //lastStatesByAgentArray[agentList.indexOf(agentHelping)].getCopy();
        //myCurrentState.agentId = agentHelping.getId(); // Replace the agent we are interested in by the agent who need to clear the path 
        //State myCurrentState = new State(State.realCoordinateById, State.realIdByCoordinate, agentHelping.getId(), agentHelping.getCurrentGoal());
        
        /*
        State myCurrentState = lastStatesByAgentArray[agentList.indexOf(agentHelping)].getCopy(); 
        myCurrentState.setLocalIdByCoordinate(State.realIdByCoordinate);
        myCurrentState.setLocalCoordinateById(State.realCoordinateById);
        */

        State myCurrentState = lastStatesByAgentArray[agentList.indexOf(agentHelping)].getCopy(); // getParent() ?
        myCurrentState.agentId = agentHelping.getId();
        ...
        System.err.println("Start computing a new plan for Helping Agent");
        Strategy strategyHelpingAgent = new Strategy.StrategyBestFirst(new Heuristic.AStar());
        ArrayList<State> plan = Search(strategyHelpingAgent, agentHelping, problemType.COMPLETE); 
        System.err.println("Plan computed.\n");
        
		if(plan != null) {
            // Show computed plan
            System.err.println("Computed plan to clear cell:\n");
            for (int i = 0; i < plan.size(); i++){
                System.err.println(plan.get(i).toString());

            }
        
            // Update plan by Agent
            /* Old verison */
            // agentHelping.appendPlan(plan);
            planByAgent.replace(agentHelping, plan);
		} else {
			System.err.println("Can't find a solution");
        }

        
        System.err.println("Show Plan of helped Agent " + agentAskingHelp.getId() + " after clearing plan computed:\n");
        for (int i = 0; i < planByAgent.get(agentAskingHelp).size(); i++){
            System.err.println(planByAgent.get(agentAskingHelp).get(i).toString());

        }
        System.err.println("Plan showed ");
        
        
        // Reinitialize basic communication attributes
		agentHelping.isClearing  = false;
        agentHelping.clearCoordinates.clear();
        agentHelping.stateToSearchStrategy = null;
		agentHelping.setOrder(true); 
        agentAskingHelp.giveOrderToAgent(agentHelping);
    ...
    /** -------------- END: Functions for communication between Agents -------------- */
    ...
    public ArrayList<State> Search(Strategy strategy, Agent agent, problemType typeOfProblem) {
        ...
        State firstState;
        if (agent.getCurrentGoal() != null && agent.stateToSearchStrategy == null) {
        ...
        } else if (agent.stateToSearchStrategy != null) {
            System.err.println("Initialization of stateToSearchStrategy for clearing");
            // firstState = new State(State.realCoordinateById, State.realIdByCoordinate, agent.getId());
            firstState = agent.stateToSearchStrategy;
            System.err.println("State used:\n" + firstState.toString());    
        ...
        while (true) {
            if (iterations == 1000) {
                // System.err.println("To much iteration");
            ...
            // System.err.println("Chosen: " + (leafState.action != null ? leafState.action.toString() : ""));
