...
public abstract class Heuristic implements Comparator<State> {
    ...
    private HashMap<BoardObject, Coordinate> getAllCoordinate(State n, String type) {
        if(type.equals("Box")){
            HashMap<BoardObject, Coordinate> coordinateByBox = new HashMap<BoardObject, Coordinate>();
        ...
        if(type.equals("Agent")){
            HashMap<BoardObject, Coordinate> coordinateByAgent = new HashMap<BoardObject, Coordinate>();
    ...
    static public double pullDistance(Coordinate c1, Coordinate c2) {
        ...
        while (!q.isEmpty()){
            ...
            Coordinate current_cord = current.getCoord();
            int current_level = current.getNumber_actions();
            ...
            for (Command.Dir direction: directions) {
                ...
                current.getCoord().setColumn(current_cord.getColumn() + Command.dirToColChange(direction));
                current.getCoord().setRow(current_cord.getRow() + Command.dirToRowChange(direction));

                // Check if next cell is Free of wall or not
                if(State.cellIsFreeFromWall(current_cord)) {

                    // Add cell Node
                    Node_PullDist next = new Node_PullDist(current.getCoord(), current.getNumber_actions()+1);

                    // Check if coordinate not visited yet
                    if (!visited.contains(next.getCoord())){

                        // Push Node in Queue and add the coordinate to visited list
                        q.add(next);
                        visited.add(next.getCoord());
                    }
	...
	public double calculate_distance(State n, String method) {
        ...
        /*
        // V1 - Don't take the assigned GOAL to BOX into account
	 ...
	 */
	private double getMinimumDistanceFromBoxToGoals(BoardObject box, Coordinate box_coordinates, String method) {
		double minimumDistance = 9999; // Initialize the minimum distance at a very high value
