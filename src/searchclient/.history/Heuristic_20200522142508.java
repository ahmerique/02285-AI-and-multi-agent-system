...
public abstract class Heuristic implements Comparator<State> {
    ...
    /**
	 * Gets a State and calculates the cost from this state to the Goal state in clearing mode
     * 
	 * @param state
	 * @return cost 
	 */
    public double clearingHeuristic(State n){
        ...
        Coordinate agent_coordinate = n.getLocalCoordinateById().get(n.getAgentId());
        // System.err.println("Coordinates of agent in clearing Heuristic: " + agent_coordinate.toString());
		for(Coordinate coord : agent.clearCoordinates){
            // System.err.println("Clear coordinates tested: " + coord.toString());
			BoardObject boardobject = State.realBoardObjectsById.get(n.getlocalIdByCoordinate().get(coord));
            Box box = null;
			...
			if(box != null && box.getColor() != null && !box.getColor().equals(agent.getColor())){
				...
				Command c = n.action;
				if(c != null && !(c.actionType == Command.Type.Move)){
					// Find if the agent is moving a box that is not the target box
					int boxRow = agent_coordinate.getRow() + Command.dirToRowChange(c.dir2);
					int boxCol = agent_coordinate.getColumn() + Command.dirToColChange(c.dir2);
					// if((boxRow == n.getLocalCoordinateById().get(box.getId()).getRow() && boxCol == n.getLocalCoordinateById().get(box.getId()).getColumn() )){
                    if((boxRow == State.realCoordinateById.get(box.getId()).getRow() && boxCol == State.realCoordinateById.get(box.getId()).getColumn() )){
						moveClearedBox = false;
                    }
                    System.err.println("moveClearedBox false");
				}
            }
        ...
        Command c2 = n.action;
        // Is there a box on the case when the agent is moving ?
		if(c2 != null && c2.actionType != Command.Type.Move){
			int boxRow = agent_coordinate.getRow() + Command.dirToRowChange(c2.dir2);
        ...
        // System.err.println("Cost computed with clearing heuristic: " + cost);
		return cost;
    }
    
/**
	 * Gets a State and return the correct heuristic depending if the agent is clearing or not
	 * @param state
	 * @return cost 
	 */
    public int h(State n) {
        ...
        if(!agent.isClearing) {
            ...
            //return (int) Math.round(clearingHeuristic(n)) + (int) Math.round(getHeuristic(n));
